...
| Lead Pipeline | a
| Analytics | already built at `/analytics` — linked from the Admin nav |

## Features

**Implemented so far:**
- Production-grade project architecture (backend + frontend + DB, cleanly separated) — Milestone 1
...
- Abstract `MessagingProvider` interface with a working `SimulatedWhatsAppProvider`, and an abstract `AIProvider` interface with a working `GeminiProvider` — Milestone 1
- Dedicated `ai/prompts/` folder — no prompt is ever hardcoded in a route — Milestone 1
- Core CRM data layer: Contacts, Conversations, Messages with real REST endpoints and a service layer (Milestone 2)
- A `/simulate/inbound` endpoint standing in for a real WhatsApp webhook — lets the whole pipeline (contact creation → conversation → message) be exercised without WhatsApp Business API credentials (Milestone 2)
- A real, WhatsApp-style inbox UI: conversation list with live previews, a message thread with bubbles, a reply composer, and a "Simulate incoming message" panel so the demo is self-contained (Milestone 2)
- Initial Alembic migration for all four tables
- **AI analysis pipeline**: every inbound message (real or simulated) triggers a single structured Gemini call that infers intent, lead status, priority, sentiment, estimated budget, a confidence score, and a running summary — written straight onto the Contact/Conversation rows (Milestone 3)
- **AI Insights panel** in the inbox: shows the live summary, intent, sentiment, estimated budget, and confidence for the open conversation (Milestone 3)
- **On-demand suggested replies**: a sparkle button in the composer drafts a reply grounded in the conversation transcript, which the agent can edit before sending — kept on-demand rather than automatic to keep AI usage proportional to actual need (Milestone 3)
